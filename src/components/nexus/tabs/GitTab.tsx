...
export function GitTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Branch strategy — FIX: render as text, NOT Mermaid (AI returns Vietnamese text description, not Mermaid syntax) */}
      {r.branchStrategy && (
        <div>
          <h3 className="flex items-center gap-2 text-base font-bold mb-3">
            <GitBranch className="w-4 h-4 text-primary" /> Branch Strategy
          </h3>
          {/* Check if branchStrategy is Mermaid syntax (starts with graph/classDiagram/etc) or plain text */}
          {(() => {
            const content = (r.branchStrategy || "").trim();
            const isMermaid = /^(graph|classDiagram|erDiagram|sequenceDiagram|flowchart|stateDiagram|gantt|pie|gitGraph)\b/.test(content);
            if (isMermaid) {
              return <MermaidRenderer code={content} id="branch" />;
            }
            // Plain text — render as paragraph
            return (
              <div className="bg-nexus-bg border border-border rounded-xl p-4">
