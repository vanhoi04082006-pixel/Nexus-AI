...
`,
  ...
  // ===== docs/TEST_PLAN.md (Agent 08 - Software Tester) =====
  const test = ts.test;
  if (test) {
    let testContent = `# Test Plan\n\n> Generated by Agent 08 - Software Tester\n\n`;
    testContent += `## Test Strategy\n\n${test.testStrategy || "N/A"}\n\n`;
    testContent += `## Unit Tests\n\n`;
    (test.unitTests || []).forEach((u) => {
      testContent += `### ${u.module}\n\n`;
      testContent += `| Test | Description | Input | Expected |\n|---|---|---|---|\n`;
      (u.cases || []).forEach((c) => {
        testContent += `| ${c.name} | ${c.desc} | ${c.input} | ${c.expected} |\n`;
      });
      testContent += `\n`;
    });
    testContent += `## Integration Tests\n\n`;
    (test.integrationTests || []).forEach((t) => {
      testContent += `### ${t.name}\n\n${t.desc}\n\n**Flow:** ${t.flow}\n\n`;
    });
    testContent += `## E2E Tests\n\n`;
    (test.e2eTests || []).forEach((t) => {
      testContent += `### ${t.name}\n\n${t.desc}\n\n**Steps:**\n`;
      (t.steps || []).forEach((s) => { testContent += `- ${s}\n`; });
      testContent += `\n`;
    });
    testContent += `## API Tests\n\n| Endpoint | Method | Cases |\n|---|---|---|\n`;
    (test.apiTests || []).forEach((t) => {
      testContent += `| ${t.endpoint} | ${t.method} | ${t.cases} |\n`;
    });
    testContent += `\n## Performance Tests\n\n| Scenario | Metric | Target |\n|---|---|---|\n`;
    (test.performanceTests || []).forEach((t) => {
      testContent += `| ${t.scenario} | ${t.metric} | ${t.target} |\n`;
    });
    if (test.bugReportTemplate) {
      testContent += `\n## Bug Report Template\n\n${test.bugReportTemplate}\n`;
    }
    files.push({ path: "docs/TEST_PLAN.md", content: testContent });
  }

  // ===== docs/SECURITY.md (Agent 09 - Security Reviewer) =====
  const security = ts.security;
  if (security) {
    let secContent = `# Security Audit\n\n> Generated by Agent 09 - Security Reviewer\n\n`;
    secContent += `## Threats\n\n| Risk | Severity | Mitigation |\n|---|---|---|\n`;
    (security.threats || []).forEach((t) => {
      secContent += `| ${t.risk} | ${t.severity} | ${t.mitigation} |\n`;
    });
    secContent += `\n## Authentication Flow\n\n${security.authFlow || "N/A"}\n\n`;
    secContent += `## Authorization Model\n\n${security.authzModel || "N/A"}\n\n`;
    secContent += `## Data Protection\n\n${security.dataProtection || "N/A"}\n\n`;
    secContent += `## OWASP Top 10 Checklist\n\n| Category | Status | Note |\n|---|---|---|\n`;
    (security.owaspChecklist || []).forEach((t) => {
      secContent += `| ${t.category} | ${t.status} | ${t.note} |\n`;
    });
    secContent += `\n## Rate Limiting\n\n${security.rateLimit || "N/A"}\n\n`;
    secContent += `## Secrets Management\n\n${security.secrets || "N/A"}\n`;
    files.push({ path: "docs/SECURITY.md", content: secContent });
  }
  ...
  summaryContent += `> Du an duoc phan tich va thiet ke boi **NEXUS AI - Multi-Agent Architect** (10 AI Agents).\n\n`;
...
export async function pushProjectToGitHub(
  ...
  // ===== Determine repo name (append short projectId for uniqueness) =====
  const slug = slugify(project.topic);
  const shortId = projectId.slice(-6);
  const repoName = project.githubRepoName || `nexus-${slug}-${shortId}`;
  ...
  try {
    // Create as PRIVATE by default (protects source code + member emails)
    const repo = await createRepo(token, repoName, description, true);
    repoFullName = repo.full_name;
    defaultBranch = repo.default_branch || "main";
    console.log(`[GitHub push] Created repo ${repoFullName} (private)`);
  } catch (err) {
  ...
  // ===== Create commit =====
  const commitMessage = `NEXUS AI: Khoi tao du an "${project.topic}"\n\n- ${files.length} files generated by 10 AI Agents\n- Analysis, HR, Sprint, Design, UML, Docs, Git workflow\n- ${tasks.length} tasks assigned to ${members.length} members`;
  const commitSha = await createCommit(token, owner, repoName, commitMessage, treeSha, parentSha);

  // ===== Strategy: if repo is empty (no main branch), create main directly.
  //        If repo already has main, create a feature branch + PR for review. =====
  let prUrl: string | null = null;
  let pushedBranch = defaultBranch;

  if (parentSha) {
    // Repo already has commits → create feature branch + PR
    const branchName = `nexus-ai/init-${projectId.slice(-6)}`;
    try {
      console.log(`[GitHub push] Creating branch ${branchName}...`);
      const branchResp = await ghFetch(`/repos/${owner}/${repoName}/git/refs`, token, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (branchResp.ok) {
        ...
        console.log(`[GitHub push] Branch created, opening PR...`);

        // Create Pull Request
        const prResp = await ghFetch(`/repos/${owner}/${repoName}/pulls`, token, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            title: `NEXUS AI: Khoi tao du an "${project.topic}"`,
            head: branchName,
            base: defaultBranch,
            body: `## NEXUS AI — Multi-Agent Project Architect\n\n**Du an:** ${project.topic}\n\n### No dung PR\n- ${files.length} files generated bởi 10 AI Agents\n- Phan tich, nhan su, sprint, thiet ke, UML, tai lieu, git\n- ${tasks.length} tasks cho ${members.length} thanh vien\n\n### Files chinh\n- \`README.md\` — Tong quan du an\n- \`docs/ARCHITECTURE.md\` — Kien truc he thong\n- \`docs/DATABASE.md\` — Schema database\n- \`docs/API_ENDPOINTS.md\` — API endpoints\n- \`docs/SPRINT_PLAN.md\` — Sprint planning\n- \`docs/TEAM.md\` — Vai tro thanh vien\n- \`docs/TASKS.md\` — Todolist chi tiet\n- \`docs/UML/*.mmd\` — UML diagrams (Mermaid)\n\n### Cach review\n1. Xem \`README.md\` de hieu tong quan\n2. Xem \`docs/TASKS.md\` de biet task cua ban\n3. Xem \`docs/CODING_CONVENTION.md\` de dong bo code\n\n---\n*Generated by NEXUS AI*`,
          }),
        });
        ...
        if (prResp.ok) {
          ...
          prUrl = pr.html_url;
          console.log(`[GitHub push] PR #${pr.number} created: ${prUrl}`);
        } else {
          // PR creation failed — push directly to main as fallback
          console.log(`[GitHub push] PR creation failed, pushing to main...`);
          await updateRef(token, owner, repoName, defaultBranch, commitSha);
        }
      } else {
        // Branch creation failed — push to main
        await updateRef(token, owner, repoName, defaultBranch, commitSha);
      }
    } catch (err) {
      console.log(`[GitHub push] Branch/PR failed, pushing to main:`, (err as Error).message);
      await updateRef(token, owner, repoName, defaultBranch, commitSha);
    }
  } else {
